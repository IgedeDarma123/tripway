...
@section('title', 'TripWay - Temukan Petualangan Terbaik di Bali')
...
@section('content')
    ...
    <section class="hero-carousel">
        ...
        <div class="hero-content">
            ...
            <form class="search-bar" action="{{ route('tours.index') }}" method="GET">
                    ...
                    <div id="search-dropdown" style="display:none; position:absolute; top:calc(100% + 8px); left:0; width:600px; background:white; border-radius:16px; box-shadow:0 16px 48px rgba(0,0,0,0.18); z-index:999; overflow:hidden; border:1px solid #e5e7eb;">
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0;">
                            ...
                            <div style="padding:16px; border-right:1px solid #f3f4f6;">
                                ...
                                <a href="{{ route('tours.index', ['destination' => $dest->slug]) }}"
                                    ...
                                    <span>{{ $dest->name }}</span>
                                    <span style="font-size:11px; color:#9ca3af; background:#f3f4f6; padding:2px 7px; border-radius:20px;">{{ $dest->tours_count }}</span>
                                </a>
                                @endforeach
                            </div>
                            <!-- Kolom Kategori -->
                            <div style="padding:16px;">
                                ...
                                <a href="{{ route('tours.index', ['category' => $cat->slug]) }}"
                                    ...
                                    onmouseover="this.style.background='#f1f5f9'; this.style.color='#1B3A4B';" onmouseout="this.style.background='transparent'; this.style.color='#374151';">
                                    <i class="fas {{ $cat->icon ?? 'fa-compass' }}" style="color:#1B3A4B; width:14px; font-size:12px;"></i>
                                    <span>{{ $cat->name }}</span>
                                </a>
                                @endforeach
                            </div>
        ...
        <div class="container">
            ...
            <div class="categories-grid">
                ...
                <a href="{{ route('tours.index', ['category' => $category->slug]) }}" class="category-card">
                    <div class="category-icon"><i class="fas {{ $category->icon ?? 'fa-compass' }}"></i></div>
                    <h3>{{ $category->name }}</h3>
                </a>
                @endforeach
            </div>
    ...
    <section class="section" style="background: var(--bg-light);">
        <div class="container">
            <div class="section-header">
                ...
                <a href="{{ route('tours.index') }}" class="see-all">Lihat Semua <i class="fas fa-chevron-right" style="font-size:10px;"></i></a>
            </div>
            <div class="destinations-grid">
                @foreach($destinations as $destination)
                <a href="{{ route('tours.index', ['destination' => $destination->slug]) }}" class="destination-card">
                    <img src="{{ $destination->image ?? 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=600' }}" alt="{{ $destination->name }}">
                    <div class="destination-overlay">
                        <h3>{{ $destination->name }}</h3>
                        <span>{{ $destination->tours_count }} aktivitas</span>
                    </div>
                </a>
                @endforeach
            </div>
