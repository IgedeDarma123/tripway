...
<div class="ticket">
    ...
    <div class="header">
        ...
        <div class="tour-meta">{{ $booking->tour->destination->name }} &nbsp;·&nbsp; {{ $booking->tour->duration }} {{ $booking->tour->duration_type == 'hours' ? 'jam' : 'hari' }}</div>
    </div>
    ...
    <div class="body">
        ...
        <table class="info">
            <tr><td class="lbl">Booking ID</td><td class="val">#{{ $booking->id }}</td></tr>
            <tr><td class="lbl">Nama</td><td class="val">{{ $booking->contact_name }}</td></tr>
            <tr><td class="lbl">Email</td><td class="val">{{ $booking->contact_email }}</td></tr>
            <tr><td class="lbl">No. HP</td><td class="val">{{ $booking->contact_phone }}</td></tr>
        </table>

        <div class="section-title">Detail Paket</div>
        <table class="info">
            <tr><td class="lbl">Paket</td><td class="val">{{ $booking->package->name ?? '-' }}</td></tr>
            <tr><td class="lbl">Tipe Perjalanan</td><td class="val">{{ ucfirst($booking->travel_type) }}</td></tr>
            @if($booking->travel_type === 'private' && $booking->groupOption)
            <tr><td class="lbl">Pilihan Grup</td><td class="val">{{ $booking->groupOption->label }}</td></tr>
            @endif
            <tr><td class="lbl">Tanggal Tour</td><td class="val">{{ $booking->travel_date->format('d M Y') }}</td></tr>
            <tr><td class="lbl">Jumlah Peserta</td><td class="val">{{ $booking->num_persons }} orang</td></tr>
        </table>

        <div class="total-box">
            <table><tr>
                <td class="tl">Total Pembayaran</td>
                <td class="tv">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</td>
            </tr></table>
        </div>
    ...
    <div class="footer">
        <table><tr>
            <td class="fl">TripWay &nbsp;·&nbsp; support@tripway.id<br>© {{ date('Y') }} TripWay. All rights reserved.</td>
            <td class="fr">
                <div class="booking-id">#{{ str_pad($booking->id, 6, '0', STR_PAD_LEFT) }}</div>
                <div class="confirmed-at">{{ $booking->payment_confirmed_at ? \Carbon\Carbon::parse($booking->payment_confirmed_at)->format('d M Y H:i') : now()->format('d M Y H:i') }}</div>
            </td>
        </tr></table>
    </div>
