<?php $__env->startSection('title', $tour->title . ' - TripWay'); ?>
...
<div class="container" style="max-width:1400px; margin:0 auto; padding:32px 48px;">
    ...
    <div style="display:grid; grid-template-columns:420px 1fr; gap:32px; align-items:start; width:100%; box-sizing:border-box;">
        ...
        <div style="min-width:0;">
                <div class="tour-gallery-carousel" style="position:relative; border-radius:var(--radius); overflow:hidden; height:420px; background:#1B3A4B; margin-bottom: 24px; box-shadow: var(--shadow-md);">
                    ...
                    <?php $__currentLoopData = $mediaList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $media): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="carousel-slide" data-index="<?php echo e($index); ?>" style="display: <?php echo e($index == 0 ? 'block' : 'none'); ?>; width:100%; height:100%; transition: opacity 0.3s ease;">
                            <?php if($media['type'] == 'video'): ?>
                                <video src="<?php echo e($media['url']); ?>" controls style="width:100%; height:100%; object-fit:contain; background:#000;"></video>
                            <?php else: ?>
                                <img src="<?php echo e($media['url']); ?>" alt="<?php echo e($tour->title); ?>" style="width:100%; height:100%; object-fit:cover;">
                            <?php endif; ?>
                        </div>
                    ...
                    <?php if(count($mediaList) > 1): ?>
                        ...
                        <div style="position:absolute; bottom:16px; left:50%; transform:translateX(-50%); display:flex; gap:8px; z-index:10; background:rgba(0,0,0,0.3); padding:8px 12px; border-radius:20px; backdrop-filter:blur(4px);">
                            <?php $__currentLoopData = $mediaList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $media): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="carousel-dot" onclick="goToSlide(<?php echo e($index); ?>)" data-index="<?php echo e($index); ?>" style="width:8px; height:8px; border-radius:50%; background:<?php echo e($index == 0 ? '#fff' : 'rgba(255,255,255,0.4)'); ?>; cursor:pointer; transition:all 0.3s ease; transform: <?php echo e($index == 0 ? 'scale(1.2)' : 'scale(1)'); ?>;"></div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <script>
                 ...
                 <div class="tour-header">
                     <div class="category"><?php echo e($tour->category->name); ?> &middot; <?php echo e($tour->destination->name); ?></div>
                     <h1 style="font-size:28px; font-weight:700; margin-bottom:12px; line-height:1.3;"><?php echo e($tour->title); ?></h1>
                     <?php if($tour->price): ?>
                     <div class="tour-price" style="font-size:18px; font-weight:700; color:#1B3A4B; margin:8px 0;">
                         Harga Tour: Rp <?php echo e(number_format($tour->price, 0, ',', '.')); ?>

                         <?php if($tour->original_price && $tour->original_price > $tour->price): ?>
                             <span style="font-size:14px; color:#9ca3af; text-decoration:line-through; margin-left:8px; font-weight:400;">Rp <?php echo e(number_format($tour->original_price, 0, ',', '.')); ?></span>
                         <?php endif; ?>
                     </div>
                     <?php endif; ?>
                     <div class="meta">
                         <span><i class="far fa-clock"></i> <?php echo e($tour->duration); ?> <?php echo e($tour->duration_type == 'hours' ? 'jam' : 'hari'); ?></span>
                         <span><i class="fas fa-users"></i> Max <?php echo e($tour->max_people); ?> orang</span>
                         <span><i class="fas fa-map-marker-alt"></i> <?php echo e($tour->destination->name); ?></span>
                     </div>
                 </div>
                ...
                <div class="tour-rating-big">
                    <span class="stars">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                            <i class="fas fa-star" style="<?php echo e($i > round($tour->rating) ? 'opacity:0.3' : ''); ?>"></i>
                        <?php endfor; ?>
                    </span>
                    <span class="score"><?php echo e($tour->rating); ?></span>
                    <span class="reviews">(<?php echo e($tour->review_count); ?> ulasan)</span>
                </div>
                <?php endif; ?>
                ...
                <div class="info-section">
                    ...
                    <ul class="highlights-list">
                        <?php $__currentLoopData = explode("\n", $tour->highlights); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $highlight): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(trim($highlight)): ?>
                            <li><i class="fas fa-check-circle"></i> <?php echo e(trim($highlight)); ?></li>
                            <?php endif; ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </ul>
                </div>
        ...
        <div style="min-width:0;">
            <div style="display:flex; gap:20px; align-items:stretch; width:100%;">
                ...
                <div style="flex:1; min-width:0; overflow:hidden;">
                    <?php echo $__env->make('tours.booking-pilihan-paket-green', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                </div>
                        ...
                        <div class="p-5 space-y-5 overflow-y-auto">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div class="bg-slate-50 rounded-xl p-3 text-center">
                                    ...
                                    <p class="font-bold text-sm text-gray-800"><?php echo e($tour->duration); ?> <?php echo e($tour->duration_type == 'hours' ? 'jam' : 'hari'); ?></p>
                                </div>
                            </div>
                            ...
                            <div id="tour-inclusions">
                                ...
                                <ul class="space-y-1.5">
                                    <?php $__currentLoopData = explode("\n", $tour->inclusions); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php if(trim($item)): ?>
                            ...
                            <div id="tour-exclusions">
                                ...
                                <ul class="space-y-1.5">
                                    <?php $__currentLoopData = explode("\n", $tour->exclusions); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php if(trim($item)): ?>
                                        <li class="flex items-start gap-2 text-sm text-gray-600">
    ...
    <div style="margin-top:48px; padding-top:32px; border-top:1px solid #e2e8f0;">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:20px;">
            <?php $__currentLoopData = $relatedTours; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route('tours.show', $rt->slug)); ?>" style="display:block; background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden; text-decoration:none; box-shadow:0 2px 8px rgba(0,0,0,0.06); transition:all 0.2s;" onmouseover="this.style.boxShadow='0 8px 24px rgba(27,58,75,0.15)'; this.style.transform='translateY(-3px)'" onmouseout="this.style.boxShadow='0 2px 8px rgba(0,0,0,0.06)'; this.style.transform='translateY(0)'">
                <div style="position:relative; height:220px; overflow:hidden;">
                    <img src="<?php echo e($rt->image ?? 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=400'); ?>" alt="<?php echo e($rt->title); ?>" style="width:100%; height:100%; object-fit:cover; display:block;">
                    <div style="position:absolute; top:10px; left:10px; background:rgba(27,58,75,0.85); color:white; font-size:10px; font-weight:700; padding:4px 10px; border-radius:20px; text-transform:uppercase;"><?php echo e($rt->category->name); ?></div>
                </div>
                <div style="padding:14px 16px;">
                    <div style="font-size:14px; font-weight:700; color:#1e293b; line-height:1.4; margin-bottom:10px;"><?php echo e($rt->title); ?></div>
                    <div style="display:flex; align-items:center; justify-content:space-between;">
                        <div style="font-size:12px; color:#64748b;"><i class="far fa-clock" style="margin-right:4px;"></i><?php echo e($rt->duration); ?> <?php echo e($rt->duration_type == 'hours' ? 'jam' : 'hari'); ?></div>
                        <div style="text-align:right;">
                            <div style="font-size:10px; color:#94a3b8;">Mulai dari</div>
                            <div style="font-size:15px; font-weight:800; color:#1B3A4B;">Rp <?php echo e(number_format($rt->price, 0, ',', '.')); ?></div>
                        </div>
                    </div>
                </div>
            </a>
...
<script>
    const tourPackages = <?php echo json_encode($tour->activePackages, 15, 512) ?>;
...
<div class="it-item">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!hasPackageItinerary) {
            const defaultRaw = <?php echo json_encode($tour->itinerary_items ?? [], 15, 512) ?>;
...
<?php echo $__env->make('layouts.tripway', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\tripway\tripway\resources\views/tours/show.blade.php ENDPATH**/ ?>