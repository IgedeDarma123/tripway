<div style="width:100%; background:white; border-radius:16px; box-shadow:0 4px 20px rgba(0,0,0,0.1); overflow:hidden; box-sizing:border-box;">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        ...
        <div class="p-4 md:p-6 pb-3 md:pb-4 border-b border-slate-200 bg-gradient-to-br from-slate-50/80 to-slate-100/80">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    ...
                    <p class="text-xs md:text-sm text-gray-500 mt-0.5"><?php echo e($tour->destination->name); ?> &middot; <?php echo e($tour->duration); ?> <?php echo e($tour->duration_type == 'hours' ? 'jam' : 'hari'); ?></p>
                </div>
        ...
        <!-- Form Booking -->
        <form id="booking-form" action="<?php echo e(route('bookings.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            ...
            <div style="margin:12px 16px 0; padding:12px 16px; background:#fef2f2; border:1px solid #fecaca; border-radius:10px;">
                ...
                <ul style="margin:0; padding-left:16px;">
                    ...
                    <li style="font-size:12px; color:#dc2626;"><?php echo e($error); ?></li>
            ...
            <div class="p-3 md:p-4 flex flex-col gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <?php if($tour->activePackages->count() > 0): ?>

                    <!-- Diskon Banner -->
                    <?php $hasDiscount = $tour->activePackages->filter(fn($p) => $p->discountPercentage() > 0)->count() > 0; ?>
                    <?php if($hasDiscount): ?>
                    <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-3 rounded-xl border border-slate-200 shadow-sm">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-xs text-gray-900">Hemat hingga <?php echo e($tour->activePackages->max(fn($p) => $p->discountPercentage())); ?>%</p>
                    ...
                    <div>
                        ...
                        <div class="space-y-1.5" id="paket-list">
                            <?php $__currentLoopData = $tour->activePackages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $package): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <button type="button"
                                ...
                                data-sharing-price="<?php echo e($package->sharing_price > 0 ? $package->sharing_price : $package->price); ?>"
                                data-name="<?php echo e($package->name); ?>"
                                data-travel-type="<?php echo e($package->travel_type); ?>"
                                data-groups="<?php echo e(json_encode($package->activeGroups->map(fn($g) => ['id'=>$g->id,'label'=>$g->label,'group_size'=>$g->group_size,'price'=>$g->price]))); ?>"
                                data-addons="<?php echo e(json_encode($package->activeAddons->map(fn($a) => ['id'=>$a->id,'name'=>$a->name,'price'=>$a->price,'description'=>$a->description]))); ?>">
                                <div class="flex justify-between items-start gap-2">
                                    <div class="min-w-0">
                                        <div class="font-bold text-xs mb-0.5"><?php echo e($package->name); ?></div>
                                        <div class="text-sm font-black text-[#1B3A4B]">Rp<?php echo e(number_format($package->price, 0, ',', '.')); ?></div>
                                        <div class="text-xs text-gray-500 mt-0.5">Maks <?php echo e($package->max_people); ?> orang</div>
                                        <?php if($package->description): ?>
                                        <div class="text-xs text-gray-400 mt-0.5"><?php echo e($package->description); ?></div>
                                        <?php endif; ?>
                                        <div class="flex gap-1 mt-1 flex-wrap">
                                            <?php if(in_array($package->travel_type, ['private','both'])): ?>
                                            <span class="text-xs px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 font-semibold">Private</span>
                                            <?php endif; ?>
                                            <?php if(in_array($package->travel_type, ['sharing','both'])): ?>
                                            <span class="text-xs px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 font-semibold">Sharing</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    <?php if($package->discountPercentage() > 0): ?>
                                    <div class="bg-[#1B3A4B] text-white text-xs font-bold px-1.5 py-0.5 rounded-full shrink-0">-<?php echo e($package->discountPercentage()); ?>%</div>
                                    <?php endif; ?>
                                </div>
                            </button>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php /**PATH D:\tripway\tripway\resources\views/tours/booking-pilihan-paket-green.blade.php ENDPATH**/ ?>